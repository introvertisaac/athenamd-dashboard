...
export default function UsersPage() {
  ...
  function openDialog(type: ActionType, user: UserListItem) {
    ...
    if (type === 'role')
      setNewRole(user.role === 'ADMIN' ? 'PATIENT' : 'ADMIN');
    ...
    <div className="space-y-6">
      ...
      {query.trim() && (
        <p className="hidden text-xs text-muted-foreground lg:block">
          Searching within this page only · server-side search coming in
          Workstream B
        </p>
      )}
                        ...
                        >
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {isLocked ? (
                                <DropdownMenuItem
                                  onClick={() => openDialog('unlock', u)}
                                >
                                  <Unlock /> Unlock account
                                </DropdownMenuItem>
                              ) : (
                                <DropdownMenuItem
                                  onClick={() => openDialog('lock', u)}
                                >
                                  <Lock /> Lock account
                                </DropdownMenuItem>
                              )}
      ...
      <Dialog open={pending?.type === 'lock'} onOpenChange={closeDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will prevent{' '}
              <span className="font-medium">{pending?.user.email}</span> from
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-medium">{pending?.user.email}</span>. This
              will grant or revoke admin access.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={pending?.type === 'delete'} onOpenChange={closeDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This is a soft delete — the user record will be marked deleted but
              not permanently removed. Their Stripe subscription will{' '}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                actionLoading || deleteConfirmEmail !== pending?.user.email
              }
            >
